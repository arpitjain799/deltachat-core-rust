...
impl ChatId {
    ...
    pub async fn get_similar_chat_ids(self, context: &Context) -> Result<Vec<(ChatId, f64)>> {
        ...
        let chat_size: BTreeMap<ChatId, f64> = context
            .sql
            .query_map(
                ...
                |rows| {
                    rows.collect::<std::result::Result<BTreeMap<ChatId, f64>, _>>()
                        .map_err(Into::into)
                },
            )
            .await
        ...
        chats_with_metrics.sort_by(|(chat_id1, metric1), (chat_id2, metric2)| {
            metric2
                .partial_cmp(metric1)
